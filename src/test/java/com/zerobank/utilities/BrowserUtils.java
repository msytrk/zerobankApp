...
public class BrowserUtils {
    ...
    // ========================= THIS ARE CREATED METHODS FOR THIS  PROJECT =============================


    //  it compare userInputList with table from app
    public static boolean verifyAccounts(List<String> fromUser,List<WebElement> fromTable) {
        boolean flag=true;

        for (int i = 0; i < fromUser.size(); i++) {
            if(fromUser.get(i).equals(fromTable.get(i).getText()))
                continue;
            else{
                flag=false;
                break;
    ...
    //  it compare userInputList with dropdown from app
    public static boolean dropdownVerify(WebElement dropdown,List<String> information) {
        boolean flag=false;

        Select stateDropdown=new Select(dropdown);
        BrowserUtils.waitFor(3);

        List<WebElement> currencyOption=stateDropdown.getOptions();
        for (WebElement webElement : currencyOption) {
            System.out.println(webElement.getText());
        }
        for (String info : information) {
            for (WebElement oneCurrency : currencyOption) {
                if(info.equals(oneCurrency.getText())){
                    System.out.println(oneCurrency.getText());
                    flag=true;
                    break;
                }
            }
    ...
    //  it choose according to Text in dropdown
    public static void chooseSelectedDropdown(WebElement typeDropdown,String option) {
        Select stateDropdown=new Select(typeDropdown);
        BrowserUtils.waitFor(3);
        stateDropdown.selectByVisibleText(option);
        stateDropdown.getFirstSelectedOption().getText();
    }

    //  it returns selected option from dropdown as a text
    public static String getSelectedDropdown(WebElement dropdown) {
    ...
    // ========================= THIS ARE READY METHODS FROM PREVIOUS PROJECT =============================
    /**
